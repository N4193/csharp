...
public class SecurityPassMaker
{
    public string GetDisplayName(TeamSupport support)
    {
        ...
        if ((support is Staff) && !(support is Security)){return $"{support.Title}";}
        if ((support is Security &&
             (support.GetType()).ToString() != "SecurityJunior" &&
             (support.GetType()).ToString() != "SecurityIntern" &&
             (support.GetType()).ToString() != "PoliceLiaison" ))
            {
